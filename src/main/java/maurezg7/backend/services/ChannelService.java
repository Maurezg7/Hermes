...
public class ChannelService {
    ...
    public List<ChannelDTO> getAllChannels(){
        ...
        return channels.stream()
                .map(c -> new ChannelDTO(c.getId() ,c.getName(), c.getDescription()))
                .toList();
    }
    ...
    @Transactional(readOnly = true)
    public  List<ChannelDTO> getAllServerChannels(Long idServer){
        if(idServer == null || idServer <= 0){
            throw new IllegalArgumentException("ID de server invalido.");
        }
        return channelRepository.findAllByServerId(idServer).stream()
    ...
    public ChannelDTO createChannel(Channel channel, Long idServer, Long idUser){
        ...
        return new ChannelDTO(savedChannel.getId(), savedChannel.getName(), savedChannel.getDescription());
    }
